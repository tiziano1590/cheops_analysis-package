...
transit_names = ['dT',
                 ...
                 'glint_scale',
                 'log_sigma',
                 'log_S0', 'log_omega0' # Gaussian Process
                ]
...
def plot_all_apertures(dataset, out_folder):
  ...
  for i, ap in enumerate(all_ap):
    ...
    # raw lc
    t, f, ef = dataset.get_lightcurve(aperture=ap,
      reject_highpoints=False,
      decontaminate=False
    )
    ax_r[i].set_title('aperture {:s} ({:.0f}px)'.format(ap, dataset.ap_rad), fontsize=6)
    ax_r[i].ticklabel_format(useOffset=False)
    ax_r[i].tick_params(labelsize=6)

    ax_r[i].errorbar(t, f, yerr=ef,
      color='C0',
      marker='o', ms=2,
      mec=out_color, mew=0.2,
      ls='',
      ecolor=out_color, elinewidth=0.4, capsize=0
    )
    # clipped lc
    t, f, ef = dataset.clip_outliers(verbose=True)
    ax_c[i].set_title('aperture {:s} ({:.0f}px)'.format(ap, dataset.ap_rad), fontsize=6)
    ax_c[i].ticklabel_format(useOffset=False)
    ax_c[i].tick_params(labelsize=6)

    ax_c[i].errorbar(t, f, yerr=ef,
      color='C0',
      marker='o', ms=2,
      mec=out_color, mew=0.2,
      ls='',
    ...
    if(i in ix):
      ax_r[i].set_xlabel(r'BJD$_\mathrm{{TDB}}-{:.0f}$'.format(dataset.lc['bjd_ref']), fontsize=8)
      ax_c[i].set_xlabel(r'BJD$_\mathrm{{TDB}}-{:.0f}$'.format(dataset.lc['bjd_ref']), fontsize=8)
    print()
  ...
  figraw.savefig(output_folder.joinpath('00_lc_raw_all_apertures.png'), bbox_inches='tight')
  figcli.savefig(output_folder.joinpath('00_lc_clipped_all_apertures.png'), bbox_inches='tight')
  plt.close('all')
...
def plot_single_lc(t, f, ef, bjd_ref):
  ...
  fig = plt.figure()
  plt.errorbar(t, f, yerr=ef,
              color='C0',
              marker='o', ms=2, mec=out_color, mew=0.5,
              ls='',
              ecolor=out_color, elinewidth=0.5, capsize=0
              )
...
def plot_custom_diagnostics(lc):
  ...
  ax[-1].plot(lc['roll_angle'], lc['contam']/np.median(lc['contam']),
              color='C1', marker='o', ms=ms, mec=mec, ls='')
  ...
  ax[-1].tick_params(axis='both', labelsize=8)
  ax[-1].plot(lc['time'], lc['roll_angle'],
              color='C1', marker='o', ms=ms, mec=mec, ls='')
  ...
  ax[-1].tick_params(axis='both', labelsize=8)
  ax[-1].plot(lc['roll_angle'], lc['bg']/np.median(lc['bg']),
              color='C1', marker='o', ms=ms, mec=mec, ls=''
              )
  ax[-1].set_xlabel('roll angle', fontsize=lsize)
  ...
  ax[-1].tick_params(axis='both', labelsize=8)
  ax[-1].plot(lc['time'], lc['bg']/np.median(lc['bg']),
              color='C1', marker='o', ms=ms, mec=mec, ls='')
  ax[-1].set_xlabel('time', fontsize=lsize)
  ax[-1].set_ylabel('bg (/median)', fontsize=lsize)
  ...
  ax[-1].tick_params(axis='both', labelsize=8)
  ax[-1].plot(lc['bg']/np.median(lc['bg']), lc['contam']/np.median(lc['contam']),
              color='C1', marker='o', ms=ms, mec=mec, ls=''
              )
  ...
  ax[-1].tick_params(axis='both', labelsize=8)
  ax[-1].plot(lc['time'], lc['contam']/np.median(lc['contam']),
              color='C1', marker='o', ms=ms, mec=mec, ls='')
  ax[-1].set_xlabel('time', fontsize=lsize)
  ax[-1].set_ylabel('contam (/median)', fontsize=lsize)
...
def computes_rms(dataset, params_best=None, glint=False, do_rms_bin=True, olog=None):
  ...
  printlog('dof = ndata ({}) - nfit ({}) = {} (ngpfit = {})'.format(
    ndata,
    nfit-ngpfit,
    ndata-nfit-ngpfit,
    ngpfit
  ...
  if(do_rms_bin):
    for binw in rms_time:
        ...
        printlog('RMS ({:8s}) = {:8.2f} +/- {:6.2f} (n_bin = {})'.format(binw_str, rms, std, nrms), olog=olog)
        statistics["flux-all (w/o GP)"]["RMS ({:8s})".format(binw_str)] = [rms, std, nrms]
  printlog("", olog=olog)
    ...
    if(do_rms_bin):
      for binw in rms_time:
          ...
          if(binw >= 1.0):
              ...
              binw_str = '{:2.0f}min'.format(binw*60.0)
          printlog('RMS ({:8s}) = {:8.2f} +/- {:6.2f} (n_bin = {})'.format(binw_str, rms, std, nrms), olog=olog)
          statistics["flux-all (w/ GP)"]["RMS ({:8s})".format(binw_str)] = [rms, std, nrms]
    printlog("", olog=olog)
  ...
  if(do_rms_bin):
    for binw in rms_time:
      ...
      if(binw >= 1.0):
          ...
          binw_str = '{:2.0f}min'.format(binw*60.0)
      printlog('RMS ({:8s}) = {:8.2f} +/- {:6.2f} (n_bin = {})'.format(binw_str, rms, std, nrms), olog=olog)
      statistics["flux-transit"]["RMS ({:8s})".format(binw_str)] = [rms, std, nrms]
  printlog("", olog=olog)
...
def plot_and_clip_lcs(datasets, apertures, out_folder, index_to_clip='all'):
  ...
  for i, dataset in enumerate(datasets):
    ...
    plt.tight_layout()
    # plt.draw()
    fig.savefig(output_folder.joinpath('00_lc_v{:02d}_raw.png'.format(i+1)), bbox_inches='tight')
    fig.savefig(output_folder.joinpath('00_lc_v{:02d}_raw.pdf'.format(i+1)), bbox_inches='tight')
    plt.close(fig)
    ...
    plt.tight_layout()
    # plt.draw()
    fig.savefig(output_folder.joinpath('01_lc_v{:02d}_clipped_outliers.png'.format(i+1)), bbox_inches='tight')
    fig.savefig(output_folder.joinpath('01_lc_v{:02d}_clipped_outliers.pdf'.format(i+1)), bbox_inches='tight')
    plt.close(fig)
...
def get_fitting_parameters(params, to_print=False):
  ...
  if(to_print):
    fmt_l = 12
    print('\nFitting parameters')
    print('{:15s} {:>12s} {:>12s} {:>12s} {:>12s}'.format('name', 'value', 'scale', 'min', 'max'))
    for i, n in enumerate(pnames):
      print('{:15s} {:>12s} {:>12s} {:>12s} {:>12s}'.format(n,
...
def get_best_parameters(result, dataset, nburn=0, dataset_type='visit', update_dataset=False):
...
#   print(pnames)
  ...
  if('log_sigma' in pnames):
    ...
    err      = 0.5*(upp-low)
    par_med['sigma_w'].value  = np.percentile(sigmaw, 50, interpolation='midpoint', axis=0)
  ...
  print('PARAMS: MEDIAN')
  par_med.pretty_print(colwidth=20, precision=8, columns=['value', 'stderr', 'vary', 'expr'])
  if('multi' in dataset_type.lower()):
    stats_med = None
  else:
  ...
  par_mle.pretty_print(colwidth=20, precision=8, columns=['value', 'stderr', 'vary', 'expr'])
  if('multi' in dataset_type.lower()):
    stats_mle = None
  else:
...
def model_plot_fit(dataset, par_fit, par_type='median', nsamples=0, flatchains=None, model_filename=None):
  ...
  if(model_filename is not None):
    ...
    out = np.column_stack((
      t_data, f_data, ef_data,
      mfit.all, mfit.tra,
      mfit.trend, mfit.glint, mfit.gp,
      res
    ))

    heado = " all flux are normalized flux\n"
    heado = "{} time = BJD_TDB - {}\n".format(heado, lc['bjd_ref'])
    heado = "{} flux_all = (flux_transit + flux_glint + flux_gp)*flux_trend\n".format(heado)
  ...
  axf.set_ylabel('flux')
  # data
  axf.errorbar(t_data, f_data, yerr=ef_data,
    color='C0',
    marker='o', ms=ms_d, mec=out_color, mew=0.5,
    ls='',
    ecolor=out_color, elinewidth=0.5, capsize=0,
    zorder=zo_d
  )
  ...
  axf.plot(t_fill, mfill.all,#mfill.all_nogp,
    color='C1',
    marker='None',
    ls='-', lw=lw_a,
    zorder=zo_a
  ...
  # data
  axd.errorbar(t_data, f_det_fit, yerr=ef_data,
    color='C0',
    marker='o', ms=ms_d, mec=out_color, mew=0.5,
    ls='',
    ecolor=out_color, elinewidth=0.5, capsize=0,
  ...
  # RESIDUALS WITH gp
    ...
    axgp.errorbar(t_data, res_nogp, yerr=ef_data,
      color='C0',
      marker='o', ms=ms_d, mec=out_color, mew=0.5,
      ls='',
      ecolor=out_color, elinewidth=0.5, capsize=0,
  ...
  if('log_sigma' in par_fit):
    ...
    ef_sc = ef_data
  axr.errorbar(t_data, res, yerr=ef_sc,
    color='C0',
    marker='o', ms=ms_d, mec=out_color, mew=0.5,
    ls='',
    ecolor=out_color, elinewidth=0.5, capsize=0,
  ...
  if(nsamples>0 and flatchains is not None):
    ...
    for n in pnames:
      ...
      rms  = np.percentile(np.abs(flatchains[:,i_fit]-cmed), cc*100, interpolation='midpoint')
      k = 5
      low, upp = cmed - k*rms, cmed + k*rms
      withinhpd = np.logical_and(flatchains[:,i_fit] >=low,
        flatchains[:,i_fit] <=upp
      )
    ...
    for idx in smp_idx:
      ...
      sfill = get_full_model(dataset, params_in=par_smp, time=t_fill) # oversampled

      # transit*trend + glint + gp model
      axf.plot(t_fill, sfill.all,
        color='C2',
        marker='None',
        ls='-', lw=lw_a,
        alpha=al_s,
...
def init_multi_model(datasets, params, pnames, gnames, tnames, do_fit=True):
  ...
  for i, dataset in enumerate(datasets):
    ...
    lmfit = dataset.lmfit_transit(
            ...
            h_1 = params['h_1'], h_2 = params['h_2'],
            # per-transit
            T_0         = tpars['T_0_{:s}'.format(v)],
            c           = tpars['c_{:s}'.format(v)],
            dfdt        = tpars['dfdt_{:s}'.format(v)],       d2fdt2     = tpars['d2fdt2_{:s}'.format(v)],
            dfdbg       = tpars['dfdbg_{:s}'.format(v)],      dfdcontam  = tpars['dfdcontam_{:s}'.format(v)],
            dfdx        = tpars['dfdx_{:s}'.format(v)],       dfdy       = tpars['dfdy_{:s}'.format(v)],
            d2fdx2      = tpars['d2fdx2_{:s}'.format(v)],     d2fdy2     = tpars['d2fdy2_{:s}'.format(v)],
            dfdsinphi   = tpars['dfdsinphi_{:s}'.format(v)],  dfdcosphi  = tpars['dfdcosphi_{:s}'.format(v)],
            dfdsin2phi  = tpars['dfdsin2phi_{:s}'.format(v)], dfdcos2phi = tpars['dfdcos2phi_{:s}'.format(v)],
            dfdsin3phi  = tpars['dfdsin3phi_{:s}'.format(v)], dfdcos3phi = tpars['dfdcos3phi_{:s}'.format(v)],
    ...
    if((tpars[ks] is not None) and (tpars[ko] is not None) and (tpars[kw] is not None)):
      if(module_celerite2):
        kernel = terms.SHOTerm(
        ...
        kernel = terms.SHOTerm(log_S0=tpars[ks].value,
  ...
  for gn in gnames:
    params_fit[gn].value = np.mean([s[gn] for s in single_pars]) # np.median([s[gn] for s in single_pars])
  for tn in tnames:
...
def components_model(pfit, pnames, gnames, tnames, params, datasets, gps, fill_gaps=False):
...
#             res = flux - flux_model[i]
      ...
      if(fill_gaps):
        pred_mean = gp.predict(res, time_fill[i], return_cov=False, return_var=False)
...
def plot_final_params(pbest, pnames, gnames, tnames, params, datasets, gps,
                      out_folder,
                      fill_gaps=True, emcee=True, pars_type='mle',
                      nsamples=0, flatchain=None):


  output_folder = Path(out_folder).resolve()

  fluxes, stats = components_model(pbest, pnames,
                  gnames, tnames, params,
                  datasets, gps, fill_gaps=fill_gaps
  )
  ...
  if(nsamples > 0 and flatchain is not None):
    ...
    for idx in smp_idx:
      psmp = flatchain[idx,:]
      smp_f, _ = components_model(psmp, pnames,
                 gnames, tnames, params,
                 datasets, gps, fill_gaps=fill_gaps
      )
  ...
  axd_phi = plt.subplot2grid((nrows-2, ncols), (0,0),
                             rowspan=2, colspan=1, fig=fig_phi
  )
  ...
  # -------------------
  # RESIDUALS IN PHASE
  axr_phi = plt.subplot2grid((nrows-2, ncols), (2,0),
                             rowspan=1, colspan=1, fig=fig_phi
  )
  ...
  for i, dataset in enumerate(datasets):
    ...
    # transit model
    axf.plot(fluxes['time_fill'][i], fluxes['model_fill'][i],
      color='black',
      marker='None',
      ls='-', lw=lw_m,
      zorder=zo_m
    ...
    # transit + trend + glint + gp model
    axf.plot(fluxes['time_fill'][i], fluxes['all_fill'][i]+fgp_fill,
      color='C1',
      marker='None',
      ls='-', lw=lw_a,
      zorder=zo_a
    ...
    axf.plot(fluxes['time_fill'][i], fluxes['trend_fill'][i]+fgl_fill+fgp_fill,
      color='C2',
      marker='None',
      ls='-', lw=lw_t,
      alpha=0.6,
    ...
    # transit model
    axd.plot(fluxes['time_fill'][i], fluxes['model_fill'][i],
      color='black',
      marker='None',
      ls='-', lw=lw_m,
      zorder=zo_m
    ...
    axd.set_ylabel(ydet)
    # in phase ...
    axd_phi.errorbar(phi, fd, yerr=ef,
      color=cphi,
      marker='o', ms=2, mec=out_color, mew=0.5,
      ls='',
      ecolor=out_color, elinewidth=0.5, capsize=0,
    ...
    axd_phi.plot(phi_fill, fluxes['model_fill'][i],
      color='black',
      marker='None',
      ls='-', lw=lw_m,
      zorder=zo_m,
    ...
    axr.set_xlabel(r'BJD$_\mathrm{{TDB}} - {}$'.format(lc['bjd_ref']))
    # in phase ...
    axr_phi.errorbar(phi, fr, yerr=ef,
      color=cphi,
      marker='o', ms=2, mec=out_color, mew=0.5,
      ls='',
      ecolor=out_color, elinewidth=0.5, capsize=0,
    ...
    fig.tight_layout()
    fig.show()
    fig.savefig(output_folder.joinpath(
    ...
    for binw in rms_time:
      ...
      if(binw >= 1.0):
        ...
        binw_str = '{:2.0f}min'.format(binw*60.0)
      print('RMS ({:8s}) = {:8.2f} +/- {:5.2f} (n_bin = {})'.format(
  ...
  fig_phi.show()
  # plt.draw()
  fig_phi.savefig(output_folder.joinpath(
...
def lnprob_wGP(pfit, pnames, gnames, tnames, params, datasets, gps):
  ...
  flux_model, _, lnlike, lnprior = multi_model(pfit, pnames,
                                    gnames, tnames, params, datasets
  )
  if (not np.all(np.isfinite(lnlike))):
    lnl = -np.inf
  else:
...
# =====================================================================
# DO EMCEE ANALYSIS!
def do_emcee_analysis(lnprob, pfit, pscale, out_folder, args=(),
                      nwalkers=64, nprerun=0, nsteps=512, nthin=4,
                      nburn=128, progress=True,
                      run_emcee=True, read_sampler=False):
  ...
  if(run_emcee):
    ...
    sampler = emcee.EnsembleSampler(nwalkers, nfit, lnprob, args=args,
      backend=backend
    )
    if progress:
      ...
      sys.stdout.flush()
    if(nprerun > 0):
      pos, _, _ = sampler.run_mcmc(pos, nprerun, store=False,
    ...
    if progress:
      ...
      sys.stdout.flush()
    _ = sampler.run_mcmc(pos, nsteps, thin_by=nthin,
      skip_initial_state_check=True,
      progress=progress
    )
  ...
  sampler_dict = sampler_to_dict(sampler,
    nburn=nburn,
    nprerun=nprerun,
    nthin=nthin
  )
...
def summary_parameters(samples, pnames, params, mle_within_hdi=True):
  ...
  print('\nFITTING PARAMETERS')
  print('{:20s} {:>11s} +/- {:>11s} ({:>11s} {:>11s}) {:>11s} +/- {:>11s} ({:>11s} {:>11s})'.format('parameter',
    'MLE', 'HDI_ERR', 'HDI_LOW', 'HDI_UPP',
    'MEDIAN', 'MEDIAN_ERR', 'CI_16p', 'CI_84p'
    )
  )
  ...
  for i, n in enumerate(pnames):
    ...
    print('{:20s} {:11s} +/- {:11s} ({:11s} {:11s}) {:11s} +/- {:11s} ({:11s} {:11s})'.format(n,
      gformat(pmle[i]), gformat(phdi_err[i]), gformat(low), gformat(upp),
      gformat(pmedian[i]), gformat(pmedian_err[i]), gformat(pci_16p[i]), gformat(pci_84p[i])
      )
    )
  ...
  mle, hdi_err, hdi_low, hdi_upp, \
    median, median_err, ci_16p, ci_84p = single_chain_summary(kchain, lnL_idx)
  print('{:20s} {:11s} +/- {:11s} ({:11s} {:11s}) {:11s} +/- {:11s} ({:11s} {:11s})'.format('k',
    gformat(mle), gformat(hdi_err), gformat(hdi_low), gformat(hdi_upp),
    gformat(median), gformat(median_err), gformat(ci_16p), gformat(ci_84p)
    )
  )
  dmle.append(mle)
  ...
  if('W' in pnames):
    ...
    mle, hdi_err, hdi_low, hdi_upp, \
      median, median_err, ci_16p, ci_84p = single_chain_summary(Wdchain, lnL_idx)
    print('{:20s} {:11s} +/- {:11s} ({:11s} {:11s}) {:11s} +/- {:11s} ({:11s} {:11s})'.format('W_d',
      gformat(mle), gformat(hdi_err), gformat(hdi_low), gformat(hdi_upp),
      gformat(median), gformat(median_err), gformat(ci_16p), gformat(ci_84p)
      )
    )
  ...
  mle, hdi_err, hdi_low, hdi_upp, \
    median, median_err, ci_16p, ci_84p = single_chain_summary(aRschain, lnL_idx)
  print('{:20s} {:11s} +/- {:11s} ({:11s} {:11s}) {:11s} +/- {:11s} ({:11s} {:11s})'.format('aRs',
    gformat(mle), gformat(hdi_err), gformat(hdi_low), gformat(hdi_upp),
    gformat(median), gformat(median_err), gformat(ci_16p), gformat(ci_84p)
    )
  )
  ...
  mle, hdi_err, hdi_low, hdi_upp, \
    median, median_err, ci_16p, ci_84p = single_chain_summary(sinichain, lnL_idx)
  print('{:20s} {:11s} +/- {:11s} ({:11s} {:11s}) {:11s} +/- {:11s} ({:11s} {:11s})'.format('sini',
    gformat(mle), gformat(hdi_err), gformat(hdi_low), gformat(hdi_upp),
    gformat(median), gformat(median_err), gformat(ci_16p), gformat(ci_84p)
    )
  )
  ...
  mle, hdi_err, hdi_low, hdi_upp, \
    median, median_err, ci_16p, ci_84p = single_chain_summary(ichain, lnL_idx)
  print('{:20s} {:11s} +/- {:11s} ({:11s} {:11s}) {:11s} +/- {:11s} ({:11s} {:11s})'.format('inc',
    gformat(mle), gformat(hdi_err), gformat(hdi_low), gformat(hdi_upp),
    gformat(median), gformat(median_err), gformat(ci_16p), gformat(ci_84p)
    )
  )
  ...
  if('f_c' in pnames and 'f_s' in pnames):
    dnames.append('ecc')
    echain = flatchain[:, pnames.index('f_c')]**2 + flatchain[:, pnames.index('f_s')]**2
    dchains.append(echain)
    mle, hdi_err, hdi_low, hdi_upp, \
      median, median_err, ci_16p, ci_84p = single_chain_summary(echain, lnL_idx)
    print('{:20s} {:11s} +/- {:11s} ({:11s} {:11s}) {:11s} +/- {:11s} ({:11s} {:11s})'.format('ecc',
      gformat(mle), gformat(hdi_err), gformat(hdi_low), gformat(hdi_upp),
      gformat(median), gformat(median_err), gformat(ci_16p), gformat(ci_84p)
      )
    )
  ...
  for p in pnames:
    if('log_sigma' in p):
      ...
      mle, hdi_err, hdi_low, hdi_upp, \
        median, median_err, ci_16p, ci_84p = single_chain_summary(sigmachain, lnL_idx)
      print('{:20s} {:11s} +/- {:11s} ({:11s} {:11s}) {:11s} +/- {:11s} ({:11s} {:11s})'.format(n,
        gformat(mle), gformat(hdi_err), gformat(hdi_low), gformat(hdi_upp),
        gformat(median), gformat(median_err), gformat(ci_16p), gformat(ci_84p)
        )
...
def trace_plot(sampler_dict, summary, out_folder):
  ...
  for i, n in enumerate(mod_names):
    ...
    print('full', end=' ')
    # plot chain
    ax.plot(chains[:,:], color='black', marker='None', ls='-', lw=0.4, alpha=0.33, zorder=9)
    # plot mle w/ error
    ax.axhspan(phdi_low[idx], phdi_upp[idx], fc='C0', ec='None', alpha=0.33, zorder=7, label='mle HDI68.27%')
    ax.axhline(pmle[idx], color='C0', ls='-', lw=1.0, alpha=1.0, zorder=8, label='mle')
    ax.axhline(pmle[idx]-phdi_err[idx], color='C0', ls='--', lw=1.0, alpha=1.0, zorder=8, label='mle+/-err')
    ax.axhline(pmle[idx]+phdi_err[idx], color='C0', ls='--', lw=1.0, alpha=1.0, zorder=8)
    # plot median w/ error
    ax.axhspan(pci_16p[idx], pci_84p[idx], fc='C1', ec='None', alpha=0.33, zorder=7, label='CI(16p-84p)')
    ax.axhline(pmedian[idx], color='C1', ls='-', lw=1.0, alpha=1.0, zorder=8, label='median')
    ax.axhline(pmedian[idx]-pmedian_err[idx], color='C1', ls='--', lw=1.0, alpha=1.0, zorder=8, label='median+/-err')
    ax.axhline(pmedian[idx]+pmedian_err[idx], color='C1', ls='--', lw=1.0, alpha=1.0, zorder=8)
    # burnin
    ax.axvline(nburn, color='darkgray', ls='-', lw=1.0, alpha=1.0, zorder=8)
    print('done')
    sys.stdout.flush()
  ...
  sys.stdout.flush()
  fig.savefig(output_folder.joinpath('08_trace_full_emcee_transit_models.png'), bbox_inches='tight')
  # print('and pdf ... ', end=' ')
  # sys.stdout.flush()
  ...
  print('posterior', end=' ')
  # plot chain
  ax.plot(lnL_all[nburn+nthin-1:nsteps:nthin, :], color='black', marker='None', ls='-', lw=0.4, alpha=0.33, zorder=9)
  # plot hdi
  ax.axhspan(lnL_low, lnL_upp, fc='C2', ec='None', alpha=0.33, zorder=7)
  # plot mle
  ax.axhline(lnL_mle, color='C0', ls='-', lw=1.0, alpha=1.0, zorder=8)
  ...
  ax.set_xlabel('nsteps posterior (burnin = {} thinning = {})'.format(nburn, nthin), fontsize=8)
  print('done')
  sys.stdout.flush()
  ...
  sys.stdout.flush()
  fig.savefig(output_folder.joinpath('08_trace_post_emcee_transit_models.png'), bbox_inches='tight')
  # print('and pdf ... ', end=' ')
  # sys.stdout.flush()
  ...
  sys.stdout.flush()
  fig.savefig(output_folder.joinpath('08_trace_full_emcee_detrending_models.png'), bbox_inches='tight')
  # print('and pdf ... ', end=' ')
  # sys.stdout.flush()
  ...
  sys.stdout.flush()
  fig.savefig(output_folder.joinpath('08_trace_post_emcee_detrending_models.png'), bbox_inches='tight')
  # print('and pdf ... ', end=' ')
  # sys.stdout.flush()
...
def print_pycheops_pars(pars, user_data=True, expr=False):
  ...
  print('\n')
  header = "{:<16s} {:>6s} {:>16s} {:>16s} {:>16s} {:>16s}".format('parameter', 'vary', 'value', 'stderr', 'min', 'max')
  if(user_data):
...
def print_analysis_stats(analysis_stats):
  ...
  h_em = "{:14s} {:>14s} {:>14s} {:>14s} {:8s}".format("EMCEE", "CHISQ", "BIC", "RMS", "err_T0_s")
  h_eg = "{:14s} {:>14s} {:>14s} {:>14s} {:8s}".format("EMCEE_GP", "CHISQ", "BIC", "RMS", "err_T0_s")
  ...
  for k_stats, v_stats in analysis_stats.items():
    ...
    l_lm = "{:14s} {:14.4f} {:14.4f} {:14.4f}".format(lmk, lmv["ChiSqr"], lmv["BIC"], lmv["RMS (unbinned)"][0])
    l_em = "{:14s} {:14.4f} {:14.4f} {:14.4f} {:8.1f}".format(emk, emv["ChiSqr"], emv["BIC"], emv["RMS (unbinned)"][0], eT0e)
    l_eg = "{:14s} {:14.4f} {:14.4f} {:14.4f} {:8.1f}".format(gpk, gpv["ChiSqr"], gpv["BIC"], gpv["RMS (unbinned)"][0], eT0g)
...
def multi_detrending_model(detrending_args):
  ...
  for analysis, det_par in detrending.items():
    ...
    _ = dataset.clip_outliers(verbose = True)
    if(clipping):
      print('Further clipping on {} with {}-sigma wrt the median.'.format(clipping[0], clipping[1]))
    ...
    #  plt.draw()
    fig.savefig(analysis_folder.joinpath('{}_04_lc_lmfit_nodetrend.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    ...
    #  plt.draw()
    fig.savefig(analysis_folder.joinpath('{}_05_lc_lmfit_detrend.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)

    fig = dataset.rollangle_plot(figsize=(5,5), fontsize=8)
    fig.savefig(analysis_folder.joinpath('{}_06_roll_angle_vs_residual.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    #  plt.draw()
    plt.close(fig)
    ...
    fig, _ = model_plot_fit(dataset, params_lm,
      par_type='lm', nsamples=0, flatchains=None,
      model_filename=analysis_folder.joinpath('{}_05_lc_lmfit.dat'.format(analysis_id))
    )
    fig.savefig(analysis_folder.joinpath('{}_05_lc_lmfit.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    ...
    if(det_par['glint_scale'] is not None):
      ...
      #  plt.draw()
      fig.savefig(analysis_folder.joinpath('{}_04_lc_lmfit_nodetrend_glint.png'.format(analysis_id)),
        bbox_inches='tight'
      )
      plt.close(fig)
      ...
      #  plt.draw()
      fig.savefig(analysis_folder.joinpath('{}_05_lc_lmfit_detrend_glint.png'.format(analysis_id)),
        bbox_inches='tight'
      )
      plt.close(fig)
      ...
      #  plt.draw()
      fig.savefig(analysis_folder.joinpath('{}_06_roll_angle_vs_residual_glint.png'.format(analysis_id)),
        bbox_inches='tight'
      )
      plt.close(fig)
      ...
      fig, _ = model_plot_fit(dataset, params_lm,
        par_type='lm+glint', nsamples=0, flatchains=None,
        model_filename=analysis_folder.joinpath('{}_05_lc_lmfit_glint.dat'.format(analysis_id))
      )
      fig.savefig(analysis_folder.joinpath('{}_05_lc_lmfit_glint.png'.format(analysis_id)),
        bbox_inches='tight'
      )
      plt.close(fig)
    # END IF GLINT
    ...
    fig = dataset.plot_emcee(title=title,
                            figsize=(5,5), fontsize=10,
                            nsamples=64,
                            binwidth=15/1440.0,
                            detrend=False
                            )
    fig.savefig(analysis_folder.joinpath('{}_07_lc_emcee_nodetrend.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    #  plt.draw()
    plt.close(fig)
    ...
    fig.savefig(analysis_folder.joinpath('{}_07_lc_emcee_detrend.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    #  plt.draw()
    plt.close(fig)
    fig = dataset.rollangle_plot(figsize=(5,5), fontsize=8)
    fig.savefig(analysis_folder.joinpath('{}_07_emcee_roll_angle_vs_residual.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    #  plt.draw()
    plt.close(fig)
    ...
    #  plt.draw()
    fig.savefig(analysis_folder.joinpath('{}_08_trace_emcee_all.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    ...
    #  plt.draw()
    fig.savefig(analysis_folder.joinpath('{}_09_corner_emcee_all.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)

    print('\n-Computing my parameters and plot models with random samples')
    params_med, stats_med, params_best, stats_mle = get_best_parameters(result, dataset, nburn=nburn)
    ...
    fig, _ = model_plot_fit(dataset, params_med,
      par_type='median', nsamples=nwalkers, flatchains=result.chain,
      model_filename=analysis_folder.joinpath('{}_07_lc_emcee_median.dat'.format(analysis_id))
    )
    fig.savefig(analysis_folder.joinpath('{}_07_lc_emcee_median.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    fig = plot_fft(dataset, params_med, star=star)
    fig.savefig(analysis_folder.joinpath('{}_07_fft_emcee_median.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)

    fig, _ = model_plot_fit(dataset, params_best,
      par_type='mle', nsamples=nwalkers, flatchains=result.chain,
      model_filename=analysis_folder.joinpath('{}_07_lc_emcee_mle.dat'.format(analysis_id))
    )
    fig.savefig(analysis_folder.joinpath('{}_07_lc_emcee_mle.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    fig = plot_fft(dataset, params_best, star=star)
    fig.savefig(analysis_folder.joinpath('{}_07_fft_emcee_mle.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    ...
    print(" T0     = {:.6f} +/- {:.6f} days = {:.6f}+/-{:.6f} BJD_TDB".format(T0_best.n, T0_best.s, BJD_best.n, BJD_best.s))
    print(" err_T0 = {:.3f} m = {:.1f} s".format(T0_best.s*cst.day2min, T0_best.s*cst.day2sec))
    print(' inc    = {:.6f} +/- {:.6f} deg'.format(inc_best.n, inc_best.s))
    print(' dur    = {:.6f} +/- {:.6f} days = {:.6f} +/- {:.6f} hour = {:.6f} +/- {:.6f} min'.format(W_best.n, W_best.s, W_h.n, W_h.s, W_m.n, W_m.s))
    print(' b      = {:.4f} +/- {:.4f}'.format(params_med['b'].value, params_med['b'].stderr))
    ...
    try:
      ...
      _, fig = massradius(m_star=Mstar, r_star=Rstar, k=k,
                            K=Kms,
                            aR=aRs, sini=si,
                            P=P,
                            jovian=True,
    ...
    #  plt.draw()
    fig.savefig(analysis_folder.joinpath('{}_10_massradius_median.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    ...
    print(' inc    = {:.6f} +/- {:.6f} deg'.format(inc_best.n, inc_best.s))
    print(' dur    = {:.6f} +/- {:.6f} days = {:.6f} +/- {:.6f} hour = {:.6f} +/- {:.6f} min'.format(W_best.n, W_best.s, W_h.n, W_h.s, W_m.n, W_m.s))
    print(' b      = {:.4f} +/- {:.4f}'.format(params_best['b'].value, params_best['b'].stderr))
    print(' k      = {:.6f} +/- {:.6f}'.format(k.n, k.s))
    print(' aRs    = {:.6f} +/- {:.6f}'.format(aRs.n, aRs.s))
    ...
    #  plt.draw()
    fig.savefig(analysis_folder.joinpath('{}_10_massradius_mle.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    print(' Saved massradius mle')

    file_emcee = save_dataset(dataset, analysis_folder.resolve(), star_name, file_key, gp=False)
    ...
    print('\n-Computing my parameters and plot models with random samples')
    params_med_gp, _, params_best_gp, _ = get_best_parameters(result_gp, dataset, nburn=nburn)
    # analysis_stats[analysis_id]["EMCEE w/ GP"] = stats_med_gp

    fig, _ = model_plot_fit(dataset, params_med_gp,
      par_type='median', nsamples=nwalkers, flatchains=result_gp.chain,
      model_filename=analysis_folder.joinpath('{}_07_lc_emcee_median_gp_train.dat'.format(analysis_id))
    )
    fig.savefig(analysis_folder.joinpath('{}_07_lc_emcee_median_gp_train.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    fig, _ = model_plot_fit(dataset, params_best_gp,
      par_type='mle', nsamples=nwalkers, flatchains=result_gp.chain,
      model_filename=analysis_folder.joinpath('{}_07_lc_emcee_mle_gp_train.dat'.format(analysis_id))
    )
    fig.savefig(analysis_folder.joinpath('{}_07_lc_emcee_mle_gp_train.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    ...
    for p in ['log_S0', 'log_omega0', 'log_sigma']:
      params_fit_gp[p].user_data = ufloat(params_best_gp[p].value, 2*params_best_gp[p].stderr)
    # Restoring the transit model parameters as free parameters
    # for p in ['T_0','D','W','b']:
      # params_fit_gp[p].set(vary=True)
    for p in in_par:
      params_fit_gp[p].set(
        vary=params_best[p].vary,
        min=in_par[p].min, max=in_par[p].max
      )
    ...
    fig.savefig(analysis_folder.joinpath('{}_07_lc_emcee_nodetrend_gp.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    fig = dataset.plot_emcee(title=title,
    ...
    fig.savefig(analysis_folder.joinpath('{}_07_lc_emcee_detrend_gp.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    fig = dataset.rollangle_plot(figsize=(5,5), fontsize=8)
    fig.savefig(analysis_folder.joinpath('{}_07_emcee_roll_angle_vs_residual_gp.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    ...
    fig = dataset.trail_plot('all') # add 'all' for all traces!
    #  plt.draw()
    fig.savefig(analysis_folder.joinpath('{}_08_trace_emcee_all_gp.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    ...
    fig = dataset.corner_plot(plotkeys='all')
    #  plt.draw()
    fig.savefig(analysis_folder.joinpath('{}_09_corner_emcee_all_gp.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)

    print('\n-Computing my parameters and plot models with random samples')
    params_med_gp, stats_med_gp, params_best_gp, stats_mle_gp = get_best_parameters(result_gp, dataset, nburn=nburn)
    ...
    fig, _ = model_plot_fit(dataset, params_med_gp,
      par_type='median', nsamples=nwalkers, flatchains=result_gp.chain,
      model_filename=analysis_folder.joinpath('{}_07_lc_emcee_median_gp.dat'.format(analysis_id))
    )
    fig.savefig(analysis_folder.joinpath('{}_07_lc_emcee_median_gp.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    fig, _ = model_plot_fit(dataset, params_best_gp,
      par_type='mle', nsamples=nwalkers, flatchains=result_gp.chain,
      model_filename=analysis_folder.joinpath('{}_07_lc_emcee_mle_gp.dat'.format(analysis_id))
    )
    fig.savefig(analysis_folder.joinpath('{}_07_lc_emcee_mle_gp.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    ...
    print(' inc    = {:.6f} +/- {:.6f} deg'.format(inc_best.n, inc_best.s))
    print(' dur    = {:.6f} +/- {:.6f} days = {:.6f} +/- {:.6f} hour = {:.6f} +/- {:.6f} min'.format(W_best.n, W_best.s, W_h.n, W_h.s, W_m.n, W_m.s))
    print(' b      = {:.4f} +/- {:.4f}'.format(params_med_gp['b'].value, params_med_gp['b'].stderr))
    print(' k      = {:.6f} +/- {:.6f}'.format(k.n, k.s))
    print(' aRs    = {:.6f} +/- {:.6f}'.format(aRs.n, aRs.s))
    ...
    #  plt.draw()
    fig.savefig(analysis_folder.joinpath('{}_10_massradius_median_gp.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    ...
    print(' inc    = {:.6f} +/- {:.6f} deg'.format(inc_best.n, inc_best.s))
    print(' dur    = {:.6f} +/- {:.6f} days = {:.6f} +/- {:.6f} hour = {:.6f} +/- {:.6f} min'.format(W_best.n, W_best.s, W_h.n, W_h.s, W_m.n, W_m.s))
    print(' b      = {:.4f} +/- {:.4f}'.format(params_best_gp['b'].value, params_best_gp['b'].stderr))
    print(' k      = {:.6f} +/- {:.6f}'.format(k.n, k.s))
    print(' aRs    = {:.6f} +/- {:.6f}'.format(aRs.n, aRs.s))
    ...
    #  plt.draw()
    fig.savefig(analysis_folder.joinpath('{}_10_massradius_mle_gp.png'.format(analysis_id)),
      bbox_inches='tight'
    )
    plt.close(fig)
    print(' Saved massradius mle with gp')

    file_emcee_gp = save_dataset(dataset, analysis_folder.resolve(), star_name, file_key, gp=True)
...
def should_I_decorr_detrending_model(detrending_args):
  ...
  for glint_value in glint_list:
    ...
    print('\n-Computing my parameters and plot models with random samples')
    params_med, _, params_best, stats_mle = get_best_parameters(result, dataset, nburn=nburn)
    ...
    print('\n-Computing my parameters and plot models with random samples')
    params_med_gp, _, params_best_gp, stats_mle_gp = get_best_parameters(result_gp, dataset, nburn=nburn)
    # analysis_stats[analysis_id]["EMCEE w/ GP"] = stats_med_gp
    analysis_stats[analysis_id]["EMCEE w/ GP"] = stats_mle_gp
...
#     smear    = smear/fluxmed
    self.lc = {'time':time, 'flux':flux, 'flux_err':flux_err,
      'bjd_ref':bjd_ref, 'table':PIPE_data, 'header':[k for k in  PIPE_data.keys()],
...
# based on get_lightcurve method of Dataset class
  def get_FITS_lightcurve(self, visit_args, transit, info, reject_highpoints=False, verbose=False):
    # start updating details within dataset
    ap_rad         = 25.0
...
#     smear    = smear/fluxmed
    self.lc = {'time':time, 'flux':flux, 'flux_err':flux_err,
      'bjd_ref':bjd_ref, 'table':FITS_data, 'header':[k for k in  FITS_data.keys()],
      'xoff':xoff, 'yoff':yoff, 'bg':bg,
      'contam':contam, 'smear':smear, 'deltaT':deltaT,
      'centroid_x':centroid_x,
      'centroid_y':centroid_y,
...
class AsciiDataset(Dataset):
  # based on get_lightcurve method of Dataset class
  def get_ascii_lightcurve(self, ascii_data, normalise=False, reject_highpoints=False, verbose=False):
    # start updating details within dataset
    ap_rad         = 0.0
    aperture       = "ascii"
    self.pipe_ver  = "ascii"
...
#     smear    = smear/fluxmed
    self.lc = {'time':time, 'flux':flux, 'flux_err':flux_err,
      'bjd_ref':bjd_ref, 'table':ascii_data, 'header':ascii_keys,
      'xoff':xoff, 'yoff':yoff, 'bg':bg,
      'contam':contam, 'smear':smear, 'deltaT':deltaT,
      'centroid_x':centroid_x,
      'centroid_y':centroid_y,
...
class CustomMultiVisit(MultiVisit):
  ...
  def __init__(self, target=None, datasets_list=None, ident=None, id_kws={'dace':True}, verbose=True):
    self.target = target
    self.datasets = []

    if target is None: return
    if datasets_list is None: return

    if(datasets_list is None):
      MultiVisit.__init__(self, target=target, datadir=None,
        ident=ident, id_kws=id_kws,
        verbose=verbose
      )
      return
...
def custom_plot_phase(M, result, title=None):
  ...
  for i, d in enumerate(M.datasets):
    ...
    axf.errorbar(ph, f, yerr=ef,
      color=dcol,
      marker='o', ms=2, mec='gray', mew=0.3,
    ...
    axd.errorbar(ph, f-f_trend, yerr=ef,
      color=dcol,
      marker='o', ms=2, mec='gray', mew=0.3,
      ecolor='gray', elinewidth=0.8, capsize=0,
      ls='',
...
def custom_plot_phase_from_model(model, title=None):
  ...
  for i in range(nvis):
    ...
    axf.errorbar(ph, f, yerr=ef,
      color=dcol,
      marker='o', ms=msdata, mec='gray', mew=0.3,
      ecolor='gray', elinewidth=0.8, capsize=0,
      ls='',
      alpha=data_alpha,
      zorder=zd+i,
  ...
  # axf.legend(loc='best', fontsize=6)
  axf.legend(loc='upper left', bbox_to_anchor=(1.001, 0.66), ncol=1, fontsize=plt.rcParams["xtick.labelsize"]-2)
...
def computes_bayes_factor(params):
  ...
  for p in params:
    u = params[p].user_data
    if(isinstance(u, UFloat)):
      if(p.startswith('dfd') or
        p.startswith('d2f') or
        p in ['ramp', 'glint_scale']):
        v = params[p].value
        s = params[p].stderr
        if s is not None:
          B = np.exp(-0.5*((v-u.n)/s)**2) * u.s/s
...
class CustomStarProperties(object):
    ...
    def __init__(self, identifier, force_download=False, dace=False,
            match_arcsec=None, configFile=None,
