...
class SingleBayes():
    ...
    def read_file(self):
        ...
        if(os.path.exists(yaml_file_in) and os.path.isfile(yaml_file_in)):
            ...
            if('D' in planet_yaml):
                ...
                read_file_status.append('ERROR: missing needed planet keyword: D or k or Rp (Rearth)')
                # sys.exit()
            planet_args['D'] = D
            planet_args['k'] = k
            ...
            if('inc' in planet_yaml and 'aRs' in planet_yaml and 'b' in planet_yaml):
                ...
                read_file_status.append('ERROR: missing needed one of these pairs/combinations: (inc, aRs) or (b, aRs) or (b, inc) or (inc, aRs, b)')
            ...
            if('T14' in planet_yaml):
                W = ufloat(planet_yaml['T14'][0], planet_yaml['T14'][1])/planet_args['P']
            else:
    ...
    def run_analysis(self):
        ...
        visit_name     = "visit_{:02d}_{:s}_{:s}_shape_ap{:s}_BF".format(visit_number,
            file_key,
            shape.lower(),
            aperture.upper()
        )
        ...
        printlog("", olog=olog)
        printlog('###############################################################################', olog=olog)
        printlog(' ANALYSIS OF A SINGLEVISIT OF CHEOPS OBSERVATION - DET. PARS. FROM BAYES FACTOR', olog=olog)
        printlog('###############################################################################', olog=olog)
        ...
        for l in read_file_status:
            if(len(l) > 0): printlog(l, olog=olog)
            if("ERROR" in l):
                error_read = True
        if(error_read):
            olog.close()
        ...
        printlog('Selected aperture = {} px ({})'.format(dataset.ap_rad, aperture), olog=olog)
        ...
        if(clipping):
            printlog('Further clipping on {} with {}-sigma wrt the median.'.format(clipping[0], clipping[1]), olog=olog)
        ...
        printlog('\nPARAMETERS OF INPUT - CUSTOM - TO BE MODIFY FOR EACH TARGET & PLANET & VISIT', olog=olog)
        ...
        in_par = Parameters()
        in_par['P']            = Parameter('P',            value = P.n,                     vary = False,    min = -np.inf,    max = np.inf,                    user_data = None)
        in_par['T_0']        = Parameter('T_0',        value = T_0[1],                vary = True,     min = T_0[0],     max = T_0[2],                    user_data = None)

        # I will randomize only fitting parameters...
        in_par['D']            = Parameter('D',            value = np.abs(np.random.normal(loc=D.n, scale=D.s)),                     vary = True,     min = 0.5*D.n,    max = 1.5*D.n,                 user_data = D)
        in_par['W']            = Parameter('W',            value = np.abs(np.random.normal(loc=W.n, scale=W.s)),                     vary = True,     min = 0.5*W.n,    max = 1.5*W.n,                 user_data = W)
        in_par['b']            = Parameter('b',            value = np.abs(np.random.normal(loc=b.n, scale=b.s)),                     vary = True,     min = 0.0,            max = 1.5,                         user_data = b)
        if(shape == 'fix'):
            for n in ['D', 'W', 'b']:
                in_par[n].vary = False
            in_par['D'].value = D.n
        ...
        in_par['h_1']        = Parameter('h_1',        value = star.h_1.n,        vary = True,     min = 0.0,            max = 1.0,                         user_data = ufloat(star.h_1.n, 0.1))
        in_par['h_2']        = Parameter('h_2',        value = star.h_2.n,        vary = True,     min = 0.0,            max = 1.0,                         user_data = ufloat(star.h_2.n, 0.1))

        in_par['f_s']        = Parameter('f_s',        value = f_s.n,                 vary = False,    min = -np.inf,    max = np.inf,                    user_data = None)
        in_par['f_c']        = Parameter('f_c',        value = f_c.n,                 vary = False,    min = -np.inf,    max = np.inf,                    user_data = None)

        in_par['logrho'] = Parameter('logrho', value = star.logrho.n, vary = True,     min = -9,             max = 6,                             user_data = star.logrho)
        in_par['c']            = Parameter('c', value = np.median(f[oot]), vary = True, min = 0.5, max = 1.5, user_data = None)
        ...
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '00_lmfit0_roll_angle_vs_residual.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)
        # best-fit plot
        params_lm0 = lmfit0.params.copy()
        fig, _ = pyca.model_plot_fit(dataset, params_lm0,
            par_type='lm', nsamples=0, flatchains=None,
            model_filename=os.path.join(visit_folder.resolve(), '00_lc_lmfit0.dat')
        )
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '00_lc_lmfit0.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)
        pyca.quick_save_params(os.path.join(visit_folder.resolve(), "00_params_lmfit0.dat"), params_lm0, dataset.lc["bjd_ref"])

        ### *** 2) determine the std of the residuals w.r.t. fitted parameters
        dataset.gp = None # force gp = None in the dataset
        stats_lm0    = pyca.computes_rms(dataset, params_best=params_lm0, glint=False, olog=olog)
        ...
        printlog("Assign prior ufloat(0,sigma_0) to all the detrending parameters", olog=olog)
        for k in det_par.keys():
            if(k not in ['ramp', 'glint_scale']):
                det_par[k] = dprior
            # printlog("{:20s} = {:.6f}".format(k, det_par[k]), olog=olog)
        printlog("dfdt and d2fdt2 will have a prior of kind N(0, sigma_0/dt),\nwhere dt=max(t)-min(t)", olog=olog)
        ...
        while True:
            ...
            printlog("Bayes Factors ( >~ 1 ==> discard parameter) sorted in descending order", olog=olog)
            ...
            if(nBFgtone == 0):
                ...
                for k, v in to_rem.items():
                    printlog("Removing parameter: {:20s} with Bayes Factor = {:7.3f}".format(k, v), olog=olog)
                    det_par[k] = None

                while_cnt += 1

        printlog("\n-DONE BAYES FACTOR SELECTION IN {} ITERATIONS".format(while_cnt), olog=olog)
        ...
        fig = dataset.rollangle_plot(figsize=plt.rcParams['figure.figsize'], fontsize=8)
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '01_lmfit_loop_roll_angle_vs_residual.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)
        # best-fit plot
        fig, _ = pyca.model_plot_fit(dataset, params_lm_loop,
            par_type='lm', nsamples=0, flatchains=None,
            model_filename=os.path.join(visit_folder.resolve(), '01_lc_lmfit_loop.dat')
        )
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '01_lc_lmfit_loop.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)
        stats_lm     = pyca.computes_rms(dataset, params_best=params_lm_loop, glint=False, olog=olog)
        pyca.quick_save_params(os.path.join(visit_folder.resolve(), "01_params_lmfit_loop.dat"), params_lm_loop, dataset.lc["bjd_ref"])
        ...
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '02_trace_emcee_all.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)
        ...
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '02_corner_emcee_all.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)

        printlog('\n-Computing my parameters and plot models with random samples', olog=olog)
        params_med, _, params_mle, stats_mle = pyca.get_best_parameters(result, dataset,
            nburn=nburn,
            dataset_type='visit',
            update_dataset=True
        )
        ...
        for p in params_med:
            printlog("{:20s} = {:20.12f} +/- {:20.12f}".format(p, params_med[p].value, params_med[p].stderr), olog=olog)
        pyca.quick_save_params(os.path.join(visit_folder.resolve(), "02_params_emcee_median.dat"), params_med, dataset.lc["bjd_ref"])

        _ = pyca.computes_rms(dataset, params_best=params_med, glint=False, olog=olog)
        fig, _ = pyca.model_plot_fit(dataset, params_med,
            par_type='median', nsamples=nwalkers, flatchains=result.chain,
            model_filename=os.path.join(visit_folder.resolve(), '02_lc_emcee_median.dat')
        )
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '02_lc_emcee_median.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)
        fig = pyca.plot_fft(dataset, params_med, star=star)
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '02_fft_emcee_median.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)
        ...
        for p in params_mle:
            printlog("{:20s} = {:20.12f} +/- {:20.12f}".format(p, params_mle[p].value, params_mle[p].stderr), olog=olog)
        pyca.quick_save_params(os.path.join(visit_folder.resolve(), "02_params_emcee_mle.dat"), params_mle, dataset.lc["bjd_ref"])

        _ = pyca.computes_rms(dataset, params_best=params_mle, glint=False, olog=olog)
        fig, _ = pyca.model_plot_fit(dataset, params_mle,
        ...
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '02_lc_emcee_mle.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)
        fig = pyca.plot_fft(dataset, params_mle, star=star)
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '02_fft_emcee_mle.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)

        file_emcee = pyca.save_dataset(dataset, visit_folder.resolve(), star_name, file_key, gp=False)
        ...
        fig = dataset.trail_plot('all') # add 'all' for all traces!
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '03_trace_emcee_gp_train.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)

        printlog('\n-Computing my parameters and plot models with random samples', olog=olog)
        params_med_gp_train, _, params_mle_gp_train, stats_mle = pyca.get_best_parameters(result_gp_train,
            dataset,
            nburn=nburn,
            dataset_type='visit',
        ...
        fig, _ = pyca.model_plot_fit(dataset, params_med_gp_train,
            par_type='median-GPtrain', nsamples=nwalkers, flatchains=result.chain,
            model_filename=os.path.join(visit_folder.resolve(), '03_lc_emcee_median_gp_train.dat')
        )
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '03_lc_emcee_median_gp_train.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)

        fig, _ = pyca.model_plot_fit(dataset, params_mle_gp_train,
            par_type='mle-GPtrain', nsamples=nwalkers, flatchains=result.chain,
            model_filename=os.path.join(visit_folder.resolve(), '03_lc_emcee_mle_gp_train.dat')
        )
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '03_lc_emcee_mle_gp_train.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)
        ...
        # log_Q = 1/sqrt(2)
        params_fit_gp.add("log_Q",
                value=np.log(1/np.sqrt(2)),
                vary=False
        )
        ...
        fig = dataset.trail_plot('all') # add 'all' for all traces!
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '04_trace_emcee_all.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)
        ...
        fig = dataset.corner_plot(plotkeys='all') 
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '04_corner_emcee_all.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)

        printlog('\n-Computing my parameters and plot models with random samples', olog=olog)
        params_med_gp, _, params_mle_gp, _ = pyca.get_best_parameters(result_gp,
            dataset,
            nburn=nburn,
            dataset_type='visit',
            update_dataset=True
        ...
        for p in params_med_gp:
            printlog("{:20s} = {:20.12f} +/- {:20.12f}".format(p, params_med_gp[p].value, params_med_gp[p].stderr), olog=olog)
        pyca.quick_save_params(os.path.join(visit_folder.resolve(), "04_params_emcee_median_gp.dat"), params_med_gp, dataset.lc["bjd_ref"])

        _ = pyca.computes_rms(dataset, params_best=params_med_gp, glint=False, olog=olog)
        fig, _ = pyca.model_plot_fit(dataset, params_med_gp,
            par_type='median w/ GP', nsamples=nwalkers, flatchains=result_gp.chain,
            model_filename=os.path.join(visit_folder.resolve(), '04_lc_emcee_median_gp.dat')
        )
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '04_lc_emcee_median_gp.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)
        fig = pyca.plot_fft(dataset, params_med_gp, star=star)
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '04_fft_emcee_median_gp.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)
        ...
        for p in params_mle_gp:
            printlog("{:20s} = {:20.12f} +/- {:20.12f}".format(p, params_mle_gp[p].value, params_mle_gp[p].stderr), olog=olog)
        pyca.quick_save_params(os.path.join(visit_folder.resolve(), "04_params_emcee_mle_gp.dat"), params_mle_gp, dataset.lc["bjd_ref"])

        _ = pyca.computes_rms(dataset, params_best=params_mle_gp, glint=False, olog=olog)
        fig, _ = pyca.model_plot_fit(dataset, params_mle_gp,
            par_type='mle w/ GP', nsamples=nwalkers, flatchains=result_gp.chain,
            model_filename=os.path.join(visit_folder.resolve(), '04_lc_emcee_mle_gp.dat')
        )
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '04_lc_emcee_mle_gp.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)
        fig = pyca.plot_fft(dataset, params_mle_gp, star=star)
        for ext in fig_ext:
            fig.savefig(os.path.join(visit_folder.resolve(), '04_fft_emcee_mle_gp.{}'.format(ext)),
                bbox_inches='tight'
            )
        plt.close(fig)

        file_emcee = pyca.save_dataset(dataset, visit_folder.resolve(), star_name, file_key, gp=True)
        printlog('-Dumped dataset into file {}'.format(file_emcee), olog=olog)
